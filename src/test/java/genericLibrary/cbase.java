...
//	create unique date time stamp
	...
	@Parameters("conf")
	@BeforeMethod
	public void launch_App(String config) throws Exception{
		DesiredCapabilities dcap = new DesiredCapabilities();
//		System.out.println(tbrowser);
		if(config.equals("system1")){
			
			dcap.setCapability("browserName", "chrome");
			dcap.setCapability("platform", "WINDOWS");			
			 
				
		}else if(config.equals("system2")){
			dcap.setCapability("browserName", "firefox");
			dcap.setCapability("platform", "WINDOWS");			

		}else if(config.equals("system3")){
			dcap.setCapability("browserName", "internet explorer");
			dcap.setCapability("platform", "WINDOWS");			
	
		}
		driver = new RemoteWebDriver(new URL("http://192.168.0.3:4444/wd/hub"),dcap);
